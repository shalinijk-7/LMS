...
from models import (
    db, Enrollment, Result, Submission, CourseCompletion,
    StudentProgress, Attendance, Assignment, LiveSession, Payment
)
...
@student_required
def analytics():
    """Student personal analytics (quizzes + assignments)."""
    enrollments = Enrollment.query.filter_by(user_id=current_user.id).all()
...
@student_required
def attendance():
    """Student attendance records (optionally filtered by course)."""
