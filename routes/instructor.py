...
from models import db, Course, Enrollment, CourseCompletion, Payment, LiveSession, Assignment, Submission, Quiz, Question, Answer, Lesson, Result
...
@instructor_required
def dashboard():
    """Instructor dashboard with course stats and revenue."""
    courses = Course.query.filter_by(instructor_id=current_user.id).all()

    total_students = sum(len(course.enrollments) for course in courses)

    # Prefer real Payment records if available, otherwise fall back to price * enrollments
    total_revenue = 0
    for course in courses:
        ...
        if payments:
            ...
            total_revenue += (course.price or 0) * len(course.enrollments)
...
@instructor_required
def sessions():
    """Schedule and list live sessions."""
    courses = Course.query.filter_by(instructor_id=current_user.id).all()
...
def quiz_results(course_id, quiz_id):
    """View quiz results for a specific quiz."""
    course = Course.query.filter_by(id=course_id, instructor_id=current_user.id).first_or_404()
...
@instructor_required
def earnings():
    """Show earnings overview for the instructor."""
    courses = Course.query.filter_by(instructor_id=current_user.id).all()
