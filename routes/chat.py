...
def chat_interface():
    ...
    if current_user.is_authenticated:
        ...
        for course in courses:
            status = CourseChatReadStatus.query.filter_by(
                user_id=current_user.id, course_id=course.id
            ).first()
            last_id = status.last_read_message_id if status else 0
            count = Message.query.filter_by(course_id=course.id).filter(
                Message.id > last_id
            ).count()
            if count > 0:
    ...
    return render_template(
        ...
        course_unread_counts=course_unread_counts
    )
...
def get_chat_history(chat_type, chat_id):
    """Returns chat history for a course or direct message."""
    if chat_type == 'course':
        messages = Message.query.filter_by(course_id=chat_id).order_by(
        ...
        if messages:
            ...
            from models import CourseChatReadStatus
            status = CourseChatReadStatus.query.filter_by(
                user_id=current_user.id, course_id=chat_id
            ).first()
            if not status:
                status = CourseChatReadStatus(
...
def get_chat_info(chat_type, chat_id):
    """Returns basic info about a chat (course or user)."""
    if chat_type == 'course':
