...
def profile():
    ...
    if request.method == 'POST':
        ...
        if theme not in ['default', 'dark', 'ocean', 'forest', 'light']:
            theme = 'default'
        ...
        # 2FA is stored on the User model (not Setting)
...
@login_required
def security():
    """Security settings page (2FA toggle etc.)."""
    if request.method == 'POST':
        # 2FA toggle on User model
        current_user.two_factor_enabled = True if request.form.get('two_factor_enabled') else False
        db.session.commit()
...
def update_theme():
    ...
    theme = data['theme']
    allowed = ['default', 'dark', 'ocean', 'forest', 'light']
