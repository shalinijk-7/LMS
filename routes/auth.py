...
def login():
    """Handles user login with optional 2FA."""
    if current_user.is_authenticated:
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            # Force 2FA for admin/instructor or if user has it enabled
...
def resend_2fa():
    """Resends the 2FA OTP with cooldown."""
    user_id = session.get('pre_2fa_user_id')
    if not user_id:
        return redirect(url_for('auth.login'))
...
def google_authorize():
    ...
    # Apply same 2FA logic for Google login
    if user.two_factor_enabled or user.role.name in ['admin', 'instructor']:
        otp = generate_otp()
        user.otp_hash = generate_password_hash(otp)
        user.otp_expires_at = datetime.utcnow() + timedelta(minutes=5)
