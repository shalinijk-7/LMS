...
from models import (
    ...
    Submission, Result, ActivityLog, Certificate
)
...
def get_analytics_data():
    ...
    active_learners = df(StudentProgress.query, StudentProgress.last_accessed).filter(
        StudentProgress.status == 'In Progress'
    ).count()
    ...
    ).filter(Attendance.attendance_date >= start_date).group_by(Attendance.status).all()
    ...
    user_growth_query = db.session.query(
        ...
        Role.name,
        func.count(User.id)
    ).join(Role).filter(User.created_at >= start_datetime).group_by(
        func.date(User.created_at), Role.name
    ).all()
    ...
    ).filter(Enrollment.enrolled_at >= start_datetime).group_by(
        func.date(Enrollment.enrolled_at)
    ).all()
    ...
    ).filter(StudentProgress.last_accessed >= start_datetime).group_by(
    ...
    ).join(Course, Course.instructor_id == User.id).group_by(User.id).order_by(
        desc('courses_count')
    ).limit(5).all()
    ...
    data = {
        ...
        'charts': {
            'user_growth': [
                {'date': str(row[0]), 'role': row[1], 'count': row[2]}
                for row in user_growth_query
            ],
            'enrollment_trends': [
                {'date': str(row[0]), 'count': row[1]} for row in enrollment_trends
            ],
            'course_performance': [
                {'title': row[0], 'enrollments': row[1]} for row in top_courses
            ],
            'revenue_trends': [
                {'date': str(row[0]), 'revenue': float(row[1])} for row in revenue_trends
            ],
        ...
        'tables': {
            ...
            'top_instructors': [
                {'name': row[0], 'courses': row[1]} for row in top_instructors
            ]
        }
    }
